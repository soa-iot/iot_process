...
layui.use(['jquery','form','upload','layer','layedit'], function(){
	...
	function isempty(){
		if($.trim($("#applypeople").val()) == ''){
			  layer.msg("上报人不能为空", {icon: 7, offset: ['170px', '450px']});
			  return true;
		}
		if($.trim($("#applypeople").val()).search(resavepeople) == -1){
			  layer.msg("上报人必须包含当前登录人", {icon: 7, offset: ['170px', '450px']});
			  return true;
		}
		
		if($("#problemtype").val() == ''){
			  layer.msg("属地单位不能为空", {icon: 7, offset: ['170px', '450px']});
			  return true;
		}
		if($("#profession").val() == ''){
			  layer.msg("所属专业不能为空", {icon: 7, offset: ['170px', '450px']});
			  return true;
		}
		if($("#problemclass").val() == ''){
			  layer.msg("问题类别不能为空", {icon: 7, offset: ['170px', '450px']});
			  return true;
		}
		if($("#rfid").val().replace(/^\s+/, '').replace(/\s+$/, '') == ''){
			  layer.msg("设备位号不能为空", {icon: 7, offset: ['170px', '450px']});
			  return true;
		}
		if($("#welName").val().replace(/^\s+/, '').replace(/\s+$/, '') == ''){
			  layer.msg("问题区域不能为空", {icon: 7, offset: ['170px', '450px']});
			  return true;
		}
		if($.trim($("#problemdescribe").val()) == ''){
			  layer.msg("问题描述不能为空", {icon: 7, offset: ['170px', '450px']});
			  return true;
		}
		return false;
	}
	...
	if(piid != null && piid != ""){
		...
		$("#problem_report").css({"display":"none"});
	}else{
		$("#problem_report").css({"display":"block"});
		$("#problem_back").css({"display":"none"});
	}
	...
	//多图片上传功能
     var uploadList = upload.render({
		 ...
		 $.ajax({
		     ...
		     ,success: function(jsonData){
		    	...
		    	if(data != undefined || data != null){
		    		...
		    		//上传问题图片
			    	uploadList.upload();
			    	layer.msg("问题上报成功",{icon: 1, offset: ['170px', '450px']});
			    	$("#problemdescribe").val("");
			    	$('#imgZmList').empty();
			    	imgCount();
		    	}else{
		    		layer.msg("问题上报失败",{icon: 2, offset: ['170px', '450px']});
		    	}
		     }
		     ,error:function(){}		       
		});
	  ...
	  //异步请求，从数据库中读取当前登录用户回退问题
	  var imgList = new Array(); 
	  if(piid != null && piid != ''){
		  $.ajax({  
		        type: 'get',  
		        url: '/iot_process/report/reload',
		        data: {"piid":piid},
		        success: function(data){ 
	        			   ...
	        			   ,done: function(res, index){
	        			     ...
	        			     if(res.state == 0 && res.message != null){
	        				     if(res.message.match("上报失败") || res.message.match("成功")){
	        				      ...
	        				      layer.msg("连接服务器失败，请检查网络是否正常", {icon: 2, offset: ['170px', '400px']});
	        				   }
...
window.onload = function(){
	...
	function windowResize(){
		...
		if(window.innerWidth > 1900){
	  		$("#problemdescribe").css({"width": "66%"});
	  	}else if(window.innerWidth > 1800){
	  		$("#problemdescribe").css({"width": "67%"});
	  	}else if(window.innerWidth > 1700){
	  		$("#problemdescribe").css({"width": "68%"});
	  	}else if(window.innerWidth > 1600){
	  		$("#problemdescribe").css({"width": "69%"});
	  	}else if(window.innerWidth > 1500){
	  		$("#problemdescribe").css({"width": "70%"});
	  	}else if(window.innerWidth > 1400){
	  		$("#problemdescribe").css({"width": "71%"});
	  	}else if(window.innerWidth > 1300){
	  		$("#problemdescribe").css({"width": "73%"});
	  	}else if(window.innerWidth > 1200){
	  		$("#problemdescribe").css({"width": "76%"});
	  	}else if(window.innerWidth > 1100){
	  		$("#problemdescribe").css({"width": "78%"});
	  	}else if(window.innerWidth > 1000){
	  		$("#problemdescribe").css({"width": "80%"});
	  	}
